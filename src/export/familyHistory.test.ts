...
describe('family history export', () => {
  it('tuottaa jäsennellyn sukuhistoria-JSONin molemmille vastaajille', () => {
    ...
    expect(exported.interview.facts).toEqual([])
    expect(exported.interview.questions[0]?.personalizedFollowUps).toEqual([])
    ...
    expect(exported.interview.questions[0]?.mark.interesting).toBe(false)
